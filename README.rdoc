= tidtools
Sorry, this manual's language is Japanese. 

tidtoolsは、TiddlyWikiをより使いやすくするためのコマンドラインツール群です。
Wiki内の検索等、様々なツールが用意されています。
RubyGem経由で簡単にインストール可能です。 
...
環境変数を指定しておけば、ファイルの場所を指定しなくても簡単に検索が出来ます。
 ...
 Tweet - 4:16:ノートのWindowsマシンだとスムーズにtidtoolsのインストールが出来た。ActiveScriptRubyの最新版(1.8.7(p249) with-winsock2 tcl-stub)にしたのが良かったのかな。
 Tweet - 4:24:tidtools、まだバグがあるよ。環境が違うことは認識したものの、kconvによる変換が上手くいっていない(何故かJISにコンバートされている？)
...
{Twitter}[http://twitter.com/tidtools]:: このツール関連の情報をしゃべくります
{blog}[http://d.hatena.ne.jp/tuto0621/]:: 開発者のブログです、他のことも書きます
